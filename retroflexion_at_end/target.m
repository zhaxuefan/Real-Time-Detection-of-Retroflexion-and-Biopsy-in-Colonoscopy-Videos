% function g = target(w)
% load('binarylabel.mat');
% feature = binaryfeaturelabel(:,1:19);
% label = binaryfeaturelabel(:,20);
% g = sum(abs(label - logsig(feature * w')));
% end
    
function target_Loss = target(weight)
...
% Auto-generated by MATLAB on 2019-04-17 21:44:20
...
load('binarylabel.mat')
trainingData = binaryfeaturelabel;
for len = 1:length(binaryfeaturelabel)
    trainingData(len,1:19) = binaryfeaturelabel(len,1:19).*weight;
end
...
target_Loss = kfoldLoss(partitionedModel, 'LossFun', 'ClassifError');
